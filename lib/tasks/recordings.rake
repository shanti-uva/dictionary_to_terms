...
namespace :dictionary_to_terms do
  namespace :recordings do
    desc "Importation of term recordings form file system.\n"+
      "Syntax: rake dictionary_to_terms:db:import:import_from_fs SOURCE=csv-file-name DIALECT=dialect_id [SOURCE_DIR=path/to/original/recordings] [TASK=task_code] [FROM=row num] [TO=row num] [FORCE=true|false] [LOG_LEVEL=0|1|2|3|4|5]\n\n"+
    ...
    task import_from_fs: :environment do
      ...
      DictionaryToTerms::RecordingsImporter.new.
        run_recording_import(from: from, to: to, dialect_id: dialect_id, filename: source, task_code: task_code, force: force, source_dir: source_dir, log_level: log_level)
    end
  end
end
